 ...
 * 
 * <p>This helper encapsulates all stamina management functionality including:
 * <ul>
 ...
 * </ul>
 * 
 * <p>All stamina values are tracked via {@link StaminaService} for consistency
...
public class StaminaHelper {
     ...
     * </ol>
     * 
     * <p>If OCR fails to read stamina, the service value remains unchanged.
     */
    public void updateStaminaFromProfile() {
        logger.debug("Opening profile to update stamina");
        ...
        if (stamina != null) {
            logger.info("Stamina parsed and stored: " + stamina);
            staminaService.setStamina(profileId, stamina);
        } else {
            logger.warn("Failed to read stamina from profile screen");
        }
     ...
     * 
     * <p>This should be called while on the deployment confirmation screen
    ...
    public Integer getSpentStamina() {
        ...
        if (spentStamina != null) {
            logger.debug("Read spent stamina: " + spentStamina);
        } else {
            logger.debug("Failed to read spent stamina from deployment screen");
        }
     ...
     * 
     * <p>If the exact stamina cost cannot be determined, uses default values:
     * <ul>
     *   <li>Solo actions: {@value DEFAULT_SOLO_STAMINA_COST} stamina</li>
     *   <li>Rally actions: {@value DEFAULT_RALLY_STAMINA_COST} stamina</li>
     * </ul>
     * 
     * @param spentStamina The actual stamina spent (from OCR), or null to use default
     * @param rally Whether this is a rally action (true) or solo action (false)
     */
    public void subtractStamina(Integer spentStamina, boolean rally) {
        if (spentStamina != null) {
            logger.info("Stamina decreased by " + spentStamina + 
                       ". Current stamina: " + (getCurrentStamina() - spentStamina));
        ...
        logger.warn("No stamina value found on deployment. Using default: " + defaultStamina);
     ...
     * 
     * <p>This is typically used when claiming stamina rewards or items.
     * 
    ...
    public void addStamina(Integer stamina) {
        ...
        logger.info("Adding " + stamina + " stamina. New total: " + 
                   (getCurrentStamina() + stamina));
        staminaService.addStamina(profileId, stamina);
    }

    /**
     * Calculates the time (in minutes) required to regenerate from current to target stamina.
     * 
     * <p>Stamina regenerates at a rate of 1 point per {@value STAMINA_REGENERATION_MINUTES_PER_POINT} minutes.
     * 
    ...
    public int staminaRegenerationTime(int currentStamina, int targetStamina) {
        ...
        logger.debug("Stamina regeneration: need " + staminaNeeded + 
                    " points, will take " + minutes + " minutes");
    ...
    /**
     * Checks if sufficient stamina and marches are available, or reschedules if not.
     * 
     ...
     * </ol>
     * 
     * <p>If either check fails, the method returns false and the caller should
    ...
    public boolean checkStaminaAndMarchesOrReschedule(
        ...
        if (currentStamina < minStaminaLevel) {
            ...
            logger.warn("Not enough stamina for expedition. (Current: " + currentStamina + 
                       "/" + minStaminaLevel + "). Rescheduling task to run in " +
        ...
        if (!marchHelper.checkMarchesAvailable()) {
            ...
            logger.warn("No marches available, rescheduling for 1 minute");
            return false;
        }
    ...
    public long parseTravelTime() {
        ...
        if (marchingTime != null) {
            ...
            logger.debug("Parsed travel time: " + seconds + " seconds");
            return seconds;
        }
        
        logger.warn("Failed to parse travel time, returning 0");
        return 0;
    }
