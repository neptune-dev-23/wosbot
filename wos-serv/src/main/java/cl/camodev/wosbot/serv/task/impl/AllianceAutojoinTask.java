 ...
 * Configures the number of queues and troop selection based on user
 * preferences.
...
public class AllianceAutojoinTask extends DelayedTask {
	...
	protected void execute() {
		...
		boolean useAllTroops = profile.getConfig(EnumConfigurationKey.ALLIANCE_AUTOJOIN_USE_ALL_TROOPS_BOOL,
				Boolean.class);
		if (useAllTroops) {
			logInfo("Configuring auto-join with all troops option");
			tapPoint(new DTOPoint(98, 376));
		} else {
			logInfo("Configuring auto-join with specific formation");
			tapPoint(new DTOPoint(98, 442));
		}
		...
		swipe(new DTOPoint(430, 600), new DTOPoint(40, 600));
		sleepTask(300);
		...
		tapRandomPoint(new DTOPoint(460, 590), new DTOPoint(497, 610),
				(queueCount - 1), 400);
		sleepTask(300);

		// Enable the auto-join feature
		logInfo("Enabling auto-join");
		tapRandomPoint(new DTOPoint(380, 1070), new DTOPoint(640, 1120));
	...
	private boolean navigateToAllianceScreen() {
		...
		DTOImageSearchResult allianceVerification = searchTemplateWithRetries(EnumTemplates.ALLIANCE_CHEST_BUTTON);
	...
	/**
	 * Handles task failure by logging a warning and rescheduling with a short retry
	 * interval
	 * 
